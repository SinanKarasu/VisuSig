...
//        updater = DisplayLink(target: self,
...
//    }
    ...
    */
    public func startRecording() -> String? {
        ...
        if checkError(status: ExtAudioFileCreateWithURL(filePath as CFURL,
                                                        kAudioFileCAFType,
                                                        format.streamDescription,
                                                        nil,
                                                        AudioFileFlags.eraseFile.rawValue,
                                                        &fileToSaveOptional),
                      mess: "Error with ExtAudioFileCreateWithURL") != noErr {
            return nil
        }
    ...
    */
    ...
    public func convertFile(filepath: URL,
        ...
        if checkError(status: ExtAudioFileOpenURL(inputFileURL,
                                                  &inputFile),
                      mess: "Error with ExtAudioFileOpenURL") != noErr {
            return nil
        }
        ...
        case .m4a(let sampleRate):
            ...
            if checkError(status: AudioFormatGetProperty(kAudioFormatProperty_FormatInfo,
                                                         ...
                                                         &size,
                                                         &outputFormat),
                          mess: "AudioFormatGetProperty failed") != noErr {
                return nil
            }
        ...
        if checkError(status: ExtAudioFileCreateWithURL(outputFileURL,
                                                        audioFileTypeID,
                                                        &outputFormat,
                                                        nil,
                                                        AudioFileFlags.eraseFile.rawValue,
                                                        &outputFile),
                      mess: "ExtAudioFileCreateWithURL failed") != noErr {
            return nil
        }
    ...
    private func convert(outputFormat: AudioStreamBasicDescription,
        ...
        while true {
            ...
            var err = checkError(status: ExtAudioFileRead(inputFile,
                                                          &frameCount,
                                                          &convertedData),
                                 mess: "ExtAudioFileRead failed")
            if err != noErr {
                return err
            }
            ...
            err = checkError(status: ExtAudioFileWrite(outputFile,
                                                       frameCount,
                                                       &convertedData),
                             mess: "ExtAudioFileWrite failed")
            if err != noErr {
                return err
            }
        }
